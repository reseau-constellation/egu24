<template>
  <v-card variant="flat" class="pa-2 mx-auto">
    ...
    <v-card-text>
      <v-img class="pb-6" width="250px" :src="observation?.image" />
      <v-text-field
        v-model="précip"
        variant="outlined"
...
const latFormatté = எண்ணை_வடிவூட்டு(
  computed(() => Number.parseFloat(props.station.coords[0].toFixed(4) || "0")),
);
const longFormatté = எண்ணை_வடிவூட்டு(
  computed(() => Number.parseFloat(props.station.coords[1].toFixed(4) || "0")),
);
